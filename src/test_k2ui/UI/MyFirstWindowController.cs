...
using test_k2ui.Unity.Runtime;
using UitkForKsp2.API;
using UnityEngine;
using UnityEngine.UIElements;

namespace test_k2ui.UI;
...
public class MyFirstWindowController : MonoBehaviour
{
    ...
    public bool IsWindowOpen
    {
        ...
        set
        {
            ...
            GameObject.Find(test_k2uiPlugin.ToolbarFlightButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);

            // Update the OAB AppBar button state
            GameObject.Find(test_k2uiPlugin.ToolbarOabButtonID)
                ?.GetComponent<UIValue_WriteBool_Toggle>()
                ?.SetValue(value);
        }
    }
